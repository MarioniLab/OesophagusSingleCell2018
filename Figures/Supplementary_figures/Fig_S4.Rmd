...
```{r}
# Removed "unknown" due to possible doublets
corrected <- corrected[,cur_sce$cell_type != "Unknown"]
cur_sce <- cur_sce[,cur_sce$cell_type != "Unknown"]
...
pdf("../../../../Dropbox (Cambridge University)/Oesophagus_single_cell/Results/Figures/Supplementary_figures/Figure_S4/NE_heatmap.pdf",
    width = 10, height = 2)
...
colour_vector["Dividing"] <- colorRampPalette(c("white", "#4DBBD5FF"))(10)[3]
...
ggsave("../../../../Dropbox (Cambridge University)/Oesophagus_single_cell/Results/Figures/Supplementary_figures/Figure_S4/GC_tsne.pdf", tsne.GC, width = 6, height = 5)
```
...
ordering <- c(which(cur_sce$cell_type == "Dividing"),
...
pdf(file = "../../../../Dropbox (Cambridge University)/Oesophagus_single_cell/Results/Figures/Supplementary_figures/Figure_S4/GC_heatmap.pdf", height = 7, width = 7)
...
colour_vector <- vector(length = length(unique(cur_sce$cell_type)))
names(colour_vector) <- unique(cur_sce$cell_type)
colour_vector["Enterocytes"] <- colorRampPalette(c("white", "#3C5488FF"))(10)[6]
colour_vector["Enterocytes_differentiated"] <- colorRampPalette(c("white", "#3C5488FF"))(10)[10]
colour_vector["Stem"] <- colorRampPalette(c("white", "#3C5488FF"))(10)[3]
colour_vector["Stem_facultative"] <- colorRampPalette(c("white", "#3C5488FF"))(10)[1]
...
ggsave("../../../../Dropbox (Cambridge University)/Oesophagus_single_cell/Results/Figures/Supplementary_figures/Figure_S4/D2_tsne.pdf", tsne.D2, width = 6, height = 5)
```
...
# Order cells by cell-type
ordering <- c(which(cur_sce$cell_type == "Stem_facultative"),
               which(cur_sce$cell_type == "Stem"),
               which(cur_sce$cell_type == "Enterocytes"),
               which(cur_sce$cell_type == "Enterocytes_differentiated"),
               which(cur_sce$cell_type == "Goblet"),
               which(cur_sce$cell_type == "Paneth"))

pdf(file = "../../../../Dropbox (Cambridge University)/Oesophagus_single_cell/Results/Figures/Supplementary_figures/Figure_S4/D2_heatmap.pdf", height = 7, width = 7)
pheatmap(for.heatmap[,ordering], cluster_rows = FALSE, color = viridis(100), 
         labels_row = genes, show_colnames = FALSE,
         annotation_col = data.frame(row.names = colnames(for.heatmap)[ordering],
                                     cell_type = cur_sce$cell_type[ordering]), 
