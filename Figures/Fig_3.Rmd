...
ggplot(data.frame(gene = logcounts(cur_sce)[rowData(cur_sce)$Symbol == "REG4",],
                  tissue = colData(cur_sce)$Tissue, 
                  patient = colData(cur_sce)$Patient)) +
  geom_boxplot(aes(tissue, gene, fill = patient))
...
for(i in conds){
  ...
  # DE
  # Note: We set the FDR to 1 to return all tested genes
...
# Order cell types
ind_order <- c(which(cur_sce$cell_type == "Undifferentiated" & cur_sce$Tissue == "GC"),
               which(cur_sce$cell_type == "Foveolar_Intermediate" & cur_sce$Tissue == "GC"),
               which(cur_sce$cell_type == "Foveolar_differentiated" & cur_sce$Tissue == "GC"),
               which(cur_sce$cell_type == "Undifferentiated" & cur_sce$Tissue == "BE"),
               which(cur_sce$cell_type == "Dividing" & cur_sce$Tissue == "BE"),
               which(cur_sce$cell_type == "Foveolar_Intermediate" & cur_sce$Tissue == "BE"),
               which(cur_sce$cell_type == "Foveolar_differentiated" & cur_sce$Tissue == "BE"))

pdf(file = "../../../Dropbox (Cambridge University)/Oesophagus_single_cell/Results/Figures/Figure_3/Marker_heatmap.pdf", height = 7, width = 8)
pheatmap(for.heatmap[,ind_order], cluster_rows = FALSE, color = viridis(100), 
         labels_row = genes, show_colnames = FALSE, cluster_cols = FALSE,
         annotation_col = data.frame(row.names = colnames(for.heatmap)[ind_order],
