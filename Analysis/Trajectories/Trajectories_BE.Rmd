...
# Find genes that correlate with branches
...
ggplot(data.frame(DC1 = dm$DC1,
                  DC3 = dm$DC3,
                  cluster = as.factor(colData(sce)$Tissue_cluster))) + 
  geom_point(aes(x = DC1, y = DC3, color = cluster))

ggplot(data.frame(DC1 = dm$DC1,
                  DC4 = dm$DC4,
                  cluster = as.factor(colData(sce)$Tissue_cluster))) + 
  geom_point(aes(x = DC1, y = DC4, color = cluster))

ggplot(data.frame(DC1 = dm$DC1,
                  DC5 = dm$DC5,
                  cluster = as.factor(colData(sce)$Tissue_cluster))) + 
  geom_point(aes(x = DC1, y = DC5, color = cluster))

# Diffusion pseudotime
dpt1 <- DPT(dm = dm, tips = c(which.max(entropy)))

plot(dpt1, root = 2, paths_to = 1, col_by ='branch')
dpt2 <- branch_divide(dpt1, 1:3)
plot(dpt2, col_by ='branch')

ggplot(data.frame(DC1 = dm$DC1,
                  DC2 = dm$DC2,
                  dpt = dpt1$DPT1093)) + 
  geom_point(aes(x = DC1, y = DC2, color = dpt)) + 
