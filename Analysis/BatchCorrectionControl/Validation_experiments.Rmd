...
I will next use the genes that are expressed in all samples.

```{r}
# Select the set of genes that are expressed in each sample
genes <- rownames(sce.list[[1]])[rowMeans(counts(sce.list[[1]])) > 0]

for(i in 2:length(sce.list)){
  genes <- intersect(genes, rownames(sce.list[[i]])[rowMeans(counts(sce.list[[i]])) > 0])
}
...
func <- paste0("fastMNN(", 
                   ...
                   ")")
corrected_3 <- eval( parse(text=func) )
corrected_3 <- t(corrected_3$corrected)
```
