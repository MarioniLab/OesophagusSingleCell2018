...
ggplot(data.frame(tSNE1 = reducedDims(tissue.list$NE)$TSNE[,1],
                  tSNE2 = reducedDims(tissue.list$NE)$TSNE[,2],
                  clusters = as.factor(colData(tissue.list$NE)$Tissue_cluster))) +
  geom_point(aes(tSNE1, tSNE2, colour = clusters))

ggplot(data.frame(tSNE1 = reducedDims(tissue.list$NE)$TSNE[,1],
                  tSNE2 = reducedDims(tissue.list$NE)$TSNE[,2],
                  clusters = as.factor(colData(tissue.list$NE)$Patient))) +
  geom_point(aes(tSNE1, tSNE2, colour = clusters))
