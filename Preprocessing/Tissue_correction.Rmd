...
ggplot(data.frame(tSNE1 = reducedDims(tissue.list$BE)$TSNE[,1],
                  tSNE2 = reducedDims(tissue.list$BE)$TSNE[,2],
                  clusters = as.factor(colData(tissue.list$BE)$Tissue_cluster))) +
  geom_point(aes(tSNE1, tSNE2, colour = clusters))

ggplot(data.frame(tSNE1 = reducedDims(tissue.list$BE)$TSNE[,1],
                  tSNE2 = reducedDims(tissue.list$BE)$TSNE[,2],
                  clusters = as.factor(colData(tissue.list$BE)$Patient))) +
  geom_point(aes(tSNE1, tSNE2, colour = clusters))
